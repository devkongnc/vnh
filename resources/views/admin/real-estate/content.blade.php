<div class="form-group">
    ...
    <div class="col-sm-2">
        {{ Form::number('product_id', $estate->product_id,
        ['id' => 'product_id', 'class' => 'form-control', 'required' => 'required', 'min' => 10000]) }}
    </div>
    <label for="status" class="col-sm-2 control-label">@lang('admin.review.visible title')</label>
    <div class="col-sm-2">
        {{ Form::select('status', array(\App\Estate::VISIBILITY_PUBLIC  => trans('admin.review.visible.public'),
        \App\Estate::VISIBILITY_PRIVATE => trans('admin.review.visible.private'),
        \App\Estate::VISIBILITY_HIDDEN  => trans('admin.review.visible.hidden')),
        $estate->status, ['class' => 'selectpicker form-control']) }}
    </div>
...
@if (!empty($estate->id))
    <div class="form-group">
        ...
        <div class="col-sm-2">
            <input type="text" name="custom_updated_at" id="updated-at" class="form-control estate-timestamps"
                   data-date-format="dd/mm/yyyy" value="{{ $estate->updated_at }}" readonly="">
        </div>
    </div>
@endif
<div class="form-group">
            ...
            @foreach(\App\Category::all() as $value)
                @if(intval($value->status) != 2)
                    <option value="{{ $value->id }}"
                            {{ in_array($value->id, explode(',', $estate->category_ids)) ? 'selected' : '' }}>
                        {{ $value->title }}</option>
                @endif
            @endforeach
        </select>
            ...
            @foreach($data['values'] as $index => $value)
                <div class="col-sm-2">
                    <div class="checkbox"><label for="{{$key}}-{{$index}}">
                            {{ Form::checkbox("term[{$key}][]", (string) $index,
                            in_array($index, (array) $estate->{$key}), ['class' => 'checkbox ' . $key]) }}
                            {{ \App\Term::getLocaleValue($value) }}</label>
                    </div>
                </div>
            @endforeach
        </div>
...
@include('partials.lang_control',
['type' => 'textarea', 'attr' => "description", 'model' => 'estate', 'class' => 'editor form-control'])
<h3 class="block-title">@lang('admin.apartment.map')</h3>
<div class="form-group">
    <div class="col-xs-12">
        <label for="lat">@lang('admin.apartment.lat')</label>
        {{ Form::text('lat', $estate->lat,
        ['id' => 'lat', 'class' => 'form-control', 'style' => 'display: inline-block; width: auto;']) }}
        &nbsp;&nbsp;&nbsp;
        <label for="lng">@lang('admin.apartment.lng')</label>
        {{ Form::text('lng', $estate->lng,
        ['id' => 'lng', 'class' => 'form-control', 'style' => 'display: inline-block; width: auto;']) }}
...
<div class="form-group">
    <div class="col-xs-12">
        <button type="button" class="btn btn-default insert-media add_media multi" data-toggle="modal"
                href="#modal-upload"><i class="fa fa-picture-o" aria-hidden="true"></i> @lang('admin.common.upload btn')
        </button>
    </div>
</div>
...
@push('scripts')
    ...
    @if(env('APP_ENV') == 'local' || env('APP_ENV') == 'develop')
        ...
        <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBy_KoPR6v-mzse7nWjricn1TTmnw9OP44&libraries=places&callback=initMap&language={{ $current_locale }}"
                async defer></script>
    @else
        {{--key for server real //office.vietnamhouse.jp--}}
        <script src="https://maps.googleapis.com/maps/api/js?key={{ env('APP_MAP_KEY') }}&libraries=places&callback=initMap&language={{ $current_locale }}"
                async defer></script>
    @endif
