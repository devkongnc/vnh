...
<div class="form-group">
    ...
    <div class="col-sm-2">
        {{ Form::select('status',
        array(\App\Category::VISIBILITY_PUBLIC  => trans('admin.review.visible.public'),
        \App\Category::VISIBILITY_PRIVATE => trans('admin.review.visible.private'),
        \App\Category::VISIBILITY_HIDDEN  => trans('admin.review.visible.hidden')),
...
<div class="form-group">
    ...
    <div class="col-sm-6">
        <div class="input-group"><span class="input-group-addon" id="basic-addon1">//{{ Request::getHost() }}
                /category/</span>
            {{ Form::text('permalink', $category->permalink,
            ['id' => 'permalink', 'class' => 'form-control', "placeholder" => "short-link",
            "required" => "true", 'pattern' => '^[\w\-]+[a-zA-Z\d]$',
            'title' => rtrim('.', trans('validation.url', ['attribute' => 'permalink']))]) }}
        </div>
    </div>
</div>
<div class="form-group">
    ...
    <div class="col-sm-10">
        ...
        ['type' => 'textarea', 'attr' => "description", 'model' => 'category', 'class' => 'editor'])
    </div>
</div>
<div class="form-group">
...
<div id='kcfinder-single-select' class="form-group">
    ...
    <div class="col-xs-10">
        ...
        <a class="remove-featured{{ $category->post_thumbnail_id === '' ? ' hidden' : '' }}">
            @lang('admin.common.remove featured image')</a>
    </div>
</div>
<div class="row">
    <div class="col-sm-6">
...
<div class="row">
    <div class="col-sm-6">
        ...
        {!! Form::text("sql_data[ids]",
        isset($category->sql_data['ids']) ? $category->sql_data['ids'] : '',
        ["class" => "form-control", "placeholder" => "Saparated by comma"]) !!}
    </div>
    <div class="col-sm-6">
        <h3 class="block-title">@lang('admin.category.price')</h3>
        {!! Form::text("sql_data[price]",
        isset($category->sql_data['price']) ? $category->sql_data['price'] : '',
        ["class" => "form-control", "placeholder" => "Price min - Price max"]) !!}
    </div>
</div>
...
@foreach($above as $key => $data)
    @if ($data['type'] !== 'text')
        @if ($key === 'city')
            <input type="hidden" name="sql_data[{{$key}}][]" value="1">
        @else
            <h3 class="block-title">{{ \App\Term::getLocaleValue($data['name']) }}</h3>
            <div class="row">
                @if ($key === 'price')
                    <div class="col-xs-12">
                        {{ Form::number("sql_data[price][]",
                        array_get($category->sql_data, 'price.0', ''), ['min' => 0, 'step' => 50, 'class' => 'price from']) }}
                        <span style="margin: 0 1em;">-</span>{{ Form::number("sql_data[price][]",
                        array_get($category->sql_data, 'price.1', ''), ['min' => 0, 'step' => 50, 'class' => 'price to']) }}
                    </div>
                @else
                    @foreach($data['values'] as $index => $value)
                        <div class="col-sm-2">
                            <div class="checkbox"><label for="{{$key}}-{{$index}}">
                                    {{ Form::checkbox("sql_data[{$key}][]",
                                    $index, in_array($index, (array) array_get($category->sql_data, $key, []), false)) }}
